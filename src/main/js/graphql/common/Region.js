'use strict';

/**
 * @author palmtale
 * @since 2017/6/9.
 */
 
 
const def = `

type Region implements Model {
  id: ID!
  name: String!
  code: String!
  sign: String!
  sort: Int!
  revoked: Boolean!
  comment: String!
  parent(filter: RegionFilter): Region
  children(filter: RegionFilter, orderBy: RegionOrderBy, skip: Int, after: String, before: String, first: Int, last: Int): RegionConnection
  createdAt: DateTime!
  updatedAt: DateTime!  
}

# An edge in a connection.
type RegionEdge {
  # The item at the end of the edge.
  model: Region!

  # A cursor for use in pagination.
  cursor: String!
}

# A connection to a list of items.
type RegionConnection {
  # Information to aid in pagination.
  pageInfo: PageInfo!

  # A list of edges.
  edges: [RegionEdge]

  # Count of filtered result set without considering pagination arguments
  count: Int!
}

input RegionFilter {
  AND: [RegionFilter!]
  OR: [RegionFilter!]
  code: String
  code_not: String
  code_in: [String!]
  code_not_in: [String!]
  code_lt: String
  code_lte: String
  code_gt: String
  code_gte: String
  code_contains: String
  code_not_contains: String
  code_starts_with: String
  code_not_starts_with: String
  code_ends_with: String
  code_not_ends_with: String
  comment: String
  comment_not: String
  comment_in: [String!]
  comment_not_in: [String!]
  comment_lt: String
  comment_lte: String
  comment_gt: String
  comment_gte: String
  comment_contains: String
  comment_not_contains: String
  comment_starts_with: String
  comment_not_starts_with: String
  comment_ends_with: String
  comment_not_ends_with: String
  createdAt: DateTime
  createdAt_not: DateTime
  createdAt_in: [DateTime!]
  createdAt_not_in: [DateTime!]
  createdAt_lt: DateTime
  createdAt_lte: DateTime
  createdAt_gt: DateTime
  createdAt_gte: DateTime
  id: ID
  id_not: ID
  id_in: [ID!]
  id_not_in: [ID!]
  id_lt: ID
  id_lte: ID
  id_gt: ID
  id_gte: ID
  id_contains: ID
  id_not_contains: ID
  id_starts_with: ID
  id_not_starts_with: ID
  id_ends_with: ID
  id_not_ends_with: ID
  name: String
  name_not: String
  name_in: [String!]
  name_not_in: [String!]
  name_lt: String
  name_lte: String
  name_gt: String
  name_gte: String
  name_contains: String
  name_not_contains: String
  name_starts_with: String
  name_not_starts_with: String
  name_ends_with: String
  name_not_ends_with: String
  revoked: Boolean
  revoked_not: Boolean
  sign: String
  sign_not: String
  sign_in: [String!]
  sign_not_in: [String!]
  sign_lt: String
  sign_lte: String
  sign_gt: String
  sign_gte: String
  sign_contains: String
  sign_not_contains: String
  sign_starts_with: String
  sign_not_starts_with: String
  sign_ends_with: String
  sign_not_ends_with: String
  sort: Int
  sort_not: Int
  sort_in: [Int!]
  sort_not_in: [Int!]
  sort_lt: Int
  sort_lte: Int
  sort_gt: Int
  sort_gte: Int
  updatedAt: DateTime
  updatedAt_not: DateTime
  updatedAt_in: [DateTime!]
  updatedAt_not_in: [DateTime!]
  updatedAt_lt: DateTime
  updatedAt_lte: DateTime
  updatedAt_gt: DateTime
  updatedAt_gte: DateTime
  children_every: RegionFilter
  children_some: RegionFilter
  children_none: RegionFilter
  parent: RegionFilter
}

enum RegionOrderBy {
  code_ASC
  code_DESC
  comment_ASC
  comment_DESC
  createdAt_ASC
  createdAt_DESC
  id_ASC
  id_DESC
  name_ASC
  name_DESC
  revoked_ASC
  revoked_DESC
  sign_ASC
  sign_DESC
  sort_ASC
  sort_DESC
  updatedAt_ASC
  updatedAt_DESC
}

input CreateRegion {
  code: String!
  comment: String!
  name: String!
  revoked: Boolean
  sign: String!
  sort: Int!
  parentId: ID
  parent: RegionparentRegion
  childrenIds: [ID!]
  children: [RegionchildrenRegion!]
}

input CreateRegionInput {
  code: String!
  comment: String!
  name: String!
  revoked: Boolean
  sign: String!
  sort: Int!
  parentId: ID
  parent: RegionparentRegion
  childrenIds: [ID!]
  children: [RegionchildrenRegion!]
  clientMutationId: String!
}

type CreateRegionPayload {
  viewer: Viewer!
  clientMutationId: String!
  region: Region
  edge: RegionEdge
  parent: Region
}

input UpdateRegion {
  code: String
  comment: String
  id: ID!
  name: String
  revoked: Boolean
  sign: String
  sort: Int
  parentId: ID
  parent: RegionparentRegion
  childrenIds: [ID!]
  children: [RegionchildrenRegion!]
}

input UpdateRegionInput {
  code: String
  comment: String
  id: ID!
  name: String
  revoked: Boolean
  sign: String
  sort: Int
  parentId: ID
  parent: RegionparentRegion
  childrenIds: [ID!]
  children: [RegionchildrenRegion!]
  clientMutationId: String!
}

type UpdateRegionPayload {
  viewer: Viewer!
  clientMutationId: String!
  region: Region
  edge: RegionEdge
  parent: Region
}

input UpdateOrCreateRegionInput {
  update: UpdateRegion!
  create: CreateRegion!
  clientMutationId: String!
}

type UpdateOrCreateRegionPayload {
  viewer: Viewer!
  clientMutationId: String!
  region: Region
  edge: RegionEdge
  parent: Region
}

input DeleteRegionInput {
  id: ID!
  clientMutationId: String!
}

type DeleteRegionPayload {
  viewer: Viewer!
  clientMutationId: String!
  region: Region
  edge: RegionEdge
  parent: Region
  deletedId: ID
}

input RegionparentRegion {
  code: String!
  comment: String!
  name: String!
  revoked: Boolean
  sign: String!
  sort: Int!
  childrenIds: [ID!]
  children: [RegionparentRegionchildrenRegion!]
}

input RegionparentRegionchildrenRegion {
  code: String!
  comment: String!
  name: String!
  revoked: Boolean
  sign: String!
  sort: Int!
  childrenIds: [ID!]
  children: [RegionparentRegionchildrenRegionchildrenRegion!]
}

input RegionparentRegionchildrenRegionchildrenRegion {
  code: String!
  comment: String!
  name: String!
  revoked: Boolean
  sign: String!
  sort: Int!
  parentId: ID
  childrenIds: [ID!]
}

input RegionchildrenRegion {
  code: String!
  comment: String!
  name: String!
  revoked: Boolean
  sign: String!
  sort: Int!
  childrenIds: [ID!]
  children: [RegionchildrenRegionchildrenRegion!]
}

input RegionchildrenRegionchildrenRegion {
  code: String!
  comment: String!
  name: String!
  revoked: Boolean
  sign: String!
  sort: Int!
  childrenIds: [ID!]
  children: [RegionchildrenRegionchildrenRegionchildrenRegion!]
}

input RegionchildrenRegionchildrenRegionchildrenRegion {
  code: String!
  comment: String!
  name: String!
  revoked: Boolean
  sign: String!
  sort: Int!
  parentId: ID
  childrenIds: [ID!]
}

type AddToRegionOnRegionPayload {
  viewer: Viewer!
  clientMutationId: String!
  childrenRegion: Region
  parentRegion: Region
  childrenRegionEdge: RegionEdge
  parentRegionEdge: RegionEdge
}

input AddToRegionOnRegionRegionInput {
  parentRegionId: ID!
  childrenRegionId: ID!
  clientMutationId: String!
}

type RemoveFromRegionOnRegionPayload {
  viewer: Viewer!
  clientMutationId: String!
  childrenRegion: Region
  parentRegion: Region
  childrenRegionEdge: RegionEdge
  parentRegionEdge: RegionEdge
}

input RemoveFromRegionOnRegionRegionInput {
  parentRegionId: ID!
  childrenRegionId: ID!
  clientMutationId: String!
}
`;

const viewer = `
    Region(code: String, id: ID, name: String, sign: String): Region
    allRegions(filter: RegionFilter, orderBy: RegionOrderBy, skip: Int, after: String, before: String, first: Int, last: Int): RegionConnection!
`;

const mutation = `
    createRegion(input: CreateRegionInput!): CreateRegionPayload
    updateRegion(input: UpdateRegionInput!): UpdateRegionPayload
    updateOrCreateRegion(input: UpdateOrCreateRegionInput!): UpdateOrCreateRegionPayload
    deleteRegion(input: DeleteRegionInput!): DeleteRegionPayload
`;

export default {def, viewer, mutation};